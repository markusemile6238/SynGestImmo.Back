...
namespace Identity.Service.Infrastructure.Data.Repositories
{
     ...
     ) : IUserCommandRepository, IUserQueryRepository
    {
        ...
        public async Task<IEnumerable<User>> GetAllUserAsync()
        {
            ...
            var query = @"
                    SELECT Id, Username,Email, UserRef, MainRoleId, MustChangePassword,EntityId, IsActive, CreatedAt, UpdatedAt  
                    FROM Users";
        ...
        public async Task<User?> GetUserByEntityIdAsync(Guid entityId)
        {
            ...
            var user = await connection.QueryFirstAsync<User?>(query, new { EntityId = entityId });
            return user;
        } 
        #endregion
